...
int main()
{
    ...
    int threadCount = -1;
#ifdef _OPENMP
    cout << "Enter in number of threads:" << endl;
    cin >> threadCount;
#else
    cout << "Open MP is not enabled, please recompile with -fopenmp" << endl;
    exit(0);
#endif
    ...
    cout << "XXXX   With Open MP                    XXX " << endl;
    ...
    clock_1.stop();
    double TimeToInitializeProcedure = clock_1.getMilliSecElapsedTime();
    ...
    for (int i=0; i < levels; i++){
        dTJ[i] = 4.0*hx*hx*pow(4.0,i)/(pi*pi*alphaX); // note hx = hy = h0 and alphaX = alphaY = alpha
    ...
    clock_2.stop();
    double TimeToAcquireSolution = clock_2.getMilliSecElapsedTime();
    ...
    cout << endl << endl;
    printf("Time to set Up Procedure            : %10.5e \n", TimeToInitializeProcedure);
    printf("Time to Acquire Solution            : %10.5e \n", TimeToAcquireSolution);
    ...
    // If need to do check, run through with doing the static version
    if(check){
        ClockIt clock_3;
        clock_3.start();
        ...
        clock_3.stop();
        double TimeToInitializeProcedure2 = clock_3.getMilliSecElapsedTime();
        ...
        clock_4.stop();
        double TimeToAcquireSolution2 = clock_4.getMilliSecElapsedTime();
        ...
        printf("\nXXXX   Variable Timestep Relaxation Output (Non OMP) XXXX \n\n");
        printf("Number of levels per sweep         : %1d \n", levels);
        printf("Number of sweeps               : %ld \n",iter);
        printf("Total Number of iterations      : %1d \n", iters_levels);
        printf("Relative residual Size (InfNorm)   : %10.5e \n",residualNormInf);
        ...
        printf("Relative solution error (InfNorm)  : %10.5e \n",relSolnError);
        cout << endl << endl;
        printf("Time to set Up Procedure            : %10.5e \n", TimeToInitializeProcedure2);
        printf("Time to Acquire Solution            : %10.5e \n", TimeToAcquireSolution2);
